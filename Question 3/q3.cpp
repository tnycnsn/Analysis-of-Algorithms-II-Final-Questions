...
public:
    ...
    bool buildDPTable(){
      ...
      M = new int *[G.getNumberOfNodes()+1];
      for(int i = 0; i < G.getNumberOfNodes()+1; i++){  //extra 1 iteration in order to check negative weight cycles
        M[i] = new int[G.getNumberOfNodes()];
      }

      for(int i = 0; i < G.getNumberOfNodes()+1; i++){  //extra 1 iteration in order to check negative weight cycles
        for(int j = 0; j < G.getNumberOfNodes(); j++){  //extra 1 iteration in order to check negative weight cycles
          M[i][j] = INT_MAX;
        }
      ...
      for(int i = 1; i < G.getNumberOfNodes()+1; i++){  //extra 1 iteration in order to check negative weight cycles
        for (std::vector<Edge>::const_iterator e = allEdges.begin(); e != allEdges.end(); ++e){
          //cout << "new  " << M[i-1][e->destinationNode] + e->weight << "\t" << M[i-1][e->sourceNode] << "\t" << (M[i-1][e->destinationNode] + e->weight < M[i-1][e->sourceNode]) << "\t"  << endl;
          if (M[i-1][e->destinationNode] != INT_MAX) M[i][e->sourceNode] = min(min(M[i-1][e->destinationNode] + e->weight, M[i-1][e->sourceNode]), M[i][e->sourceNode]);
          else M[i][e->sourceNode] = min(M[i-1][e->sourceNode], M[i][e->sourceNode]);
        }
      }

      graphContainsNegativeWeightCycle = false;
      for (int i = 0; i < G.getNumberOfNodes(); i++){
        if (M[G.getNumberOfNodes()][i] != M[G.getNumberOfNodes()-1][i]){
    ...
    void printShortestPathBetweenSourceAndDestination(){
        ...
        if (graphContainsNegativeWeightCycle) cout << "Graph contains negative weight cycle!" << endl;
        else{
          ...
          cout << "Shortest path from " << sourceNode << " to " << destinationNode << ": " << currentNode;  //print paths's first node
          while (currentNode != destinationNode){
            vector<Edge> outgoingEdges = G.getOutgoingEdges(currentNode); //find outgoing edges from the current node
            for (std::vector<Edge>::const_iterator e = outgoingEdges.begin(); e != outgoingEdges.end(); ++e){
              //if Graph doesnt contain neg weighted cycle last row of the table is same as last meaningful BellmanFord Table row
              if (M[G.getNumberOfNodes()-1][currentNode] == (M[G.getNumberOfNodes()-1][e->destinationNode] + e->weight)){
                currentNode = e->destinationNode;
                cout << " -> " << currentNode;   //print each visited node
              }
            }
          }
          cout << endl;
